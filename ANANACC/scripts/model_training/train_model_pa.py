# scripts/model_training/train_model_pa.py
import pandas as pd
import numpy as np
...
def train_model(ticker, X_train, y_train):
    """Обучает и сохраняет модель PassiveAggressiveClassifier."""
    scaler = StandardScaler()
    X_train_scaled = scaler.fit_transform(X_train)

    model = PassiveAggressiveClassifier(**MODEL_PARAMS["PassiveAggressiveClassifier"])
...
def main(ticker="GAZP"):
    print(f"=== Обучение модели PassiveAggressive для {ticker} ===")
