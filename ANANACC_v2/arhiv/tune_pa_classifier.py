# tune_pa_classifier.py (полная прокачанная версия)
import pandas as pd
import numpy as np
...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', handlers=[logging.FileHandler('tune_pa_classifier.log'), logging.StreamHandler()])
...
def perform_grid_search(X_train, y_train):
    """Выполняет GridSearchCV для PassiveAggressiveClassifier."""
    logger.info("\n--- Running GridSearchCV for PassiveAggressiveClassifier ---")
...
def main():
    """Основная функция."""
    logger.info("Starting search for best hyperparameters for PassiveAggressiveClassifier with GridSearchCV...")
    ...
    results_df.to_csv('grid_search_pa_results.csv', index=False, encoding='utf-8-sig')
    logger.info("\nGridSearchCV results for PassiveAggressiveClassifier saved to 'grid_search_pa_results.csv'.")
    logger.info("\nSearch for best hyperparameters for PassiveAggressiveClassifier completed.")
