# train_model_pa.py (полная прокачанная версия)
import pandas as pd
import numpy as np
...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', handlers=[logging.FileHandler('train_model_pa.log'), logging.StreamHandler()])
...
def perform_incremental_learning(model, X_test, y_test, test_dates):
    ...
    log_file = os.path.join(config['logs_dir'], 'incremental_learning_log_pa.csv')
    log_df.to_csv(log_file, index=False, encoding='utf-8-sig')
    logger.info(f"Incremental learning log (PassiveAggressiveClassifier) saved to '{log_file}'.")

def main():
    """Основная функция."""
    logger.info("Starting training of PassiveAggressiveClassifier model with tuning and incremental learning...")
    ...
    logger.info("\nTraining of PassiveAggressiveClassifier model with tuning and incremental learning completed.")
