# tune_passive_aggressive.py (полная прокачанная версия)
import pandas as pd
import numpy as np
...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', handlers=[logging.FileHandler('tune_passive_aggressive.log'), logging.StreamHandler()])
...
def main():
    ...
    logger.info("\nGridSearchCV results for SGDClassifier saved to 'grid_search_sgd_results.csv'.")
    logger.info("\nSearch for best hyperparameters for SGDClassifier completed.")
