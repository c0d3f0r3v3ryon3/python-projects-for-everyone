# tune_pa_classifier_extended.py
import pandas as pd
import numpy as np
from sklearn.linear_model import PassiveAggressiveClassifier
from sklearn.preprocessing import StandardScaler
from sklearn.model_selection import GridSearchCV, train_test_split
from sklearn.metrics import accuracy_score, classification_report, precision_score, recall_score, f1_score  # Добавлены недостающие импорты
import os
import time
...
# --- Расширенная сетка гиперпараметров для PassiveAggressiveClassifier ---
# Основные параметры PassiveAggressiveClassifier
PARAM_GRID = {
    'C': [0.01, 0.1, 1.0, 10.0], # Коэффициент регуляризации
    'loss': ['hinge', 'squared_hinge'], # Функция потерь
    'average': [False, True], # Усреднение весов
    'fit_intercept': [True], # Обычно True
    'max_iter': [1000, 2000], # Максимальное количество итераций
    'tol': [1e-3, 1e-4], # Порог останова
    # Дополнительные параметры
    'shuffle': [True], # Перемешивать ли данные перед каждой итерацией
...
def perform_grid_search_optimized(X_train, y_train):
    ...
    print(f"\n--- Запуск оптимизированного GridSearchCV для PassiveAggressiveClassifier ---")
    ...
    base_model = PassiveAggressiveClassifier(random_state=RANDOM_STATE)
    ...
    print("Прогресс будет отображаться...")
...
def analyze_feature_importance(best_model, feature_names, top_n=20):
    ...
    if hasattr(best_model, 'coef_'):
        ...
        feature_imp_df.to_csv('feature_importance_pa_extended.csv', index=False, encoding='utf-8-sig')
...
def detailed_evaluation(best_model, scaler, X_test, y_test):
    ...
    print("\n--- Оценка модели на тестовой выборке (с масштабированием) ---")
    if len(y_test) == 0:
...
def save_results(grid_search, test_accuracy, feature_imp_df, duration):
    ...
    top_10.to_csv('grid_search_pa_extended_top10.csv', index=False, encoding='utf-8-sig')

    results_df.to_csv('grid_search_pa_extended_results.csv', index=False, encoding='utf-8-sig')

    print("\nРезультаты сохранены:")
    print("- Основные результаты: grid_search_pa_extended_results.csv")
    print("- Топ-10 комбинаций: grid_search_pa_extended_top10.csv")
    if feature_imp_df is not None:
        print("- Важность признаков: feature_importance_pa_extended.csv")

def main():
    """Основная функция."""
    print("=== ОПТИМИЗИРОВАННЫЙ ПОИСК ГИПЕРПАРАМЕТРОВ ДЛЯ PASSIVEAGGRESSIVECLASSIFIER (РАСШИРЕННЫЙ) ===")
    ...
    test_acc, test_prec, test_rec, test_f1 = detailed_evaluation(best_model, scaler, X_test, y_test)
    ...
    save_results(grid_search_result, test_acc, feature_imp_df, search_duration)
