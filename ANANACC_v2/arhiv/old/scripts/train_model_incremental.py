...
# --- Лучшие гиперпараметры, найденные с помощью GridSearchCV ---
# Обновлены параметры из grid_search_sgd_results.csv
BEST_PARAMS = {
    'loss': 'hinge',         # Изменено с 'log_loss'
    'alpha': 0.001,          # Изменено с 0.0001
    'learning_rate': 'constant', # Изменено с 'adaptive'
    'eta0': 0.1,             # Изменено с 0.01
    'penalty': 'l2',
    'random_state': RANDOM_STATE,
    'max_iter': 1000,        # Увеличен с 1
    'tol': 1e-3,
    # Добавлены недостающие параметры для SGDClassifier
    'fit_intercept': True,
    'shuffle': True,
    'verbose': 0,
    'epsilon': 0.1,
...
def load_and_prepare_data(filename):
    ...
    # --- Разделение данных ---
    # Сначала получим индексы очищенных данных в оригинальном df
    clean_indices = X_clean.index
    # Используем эти индексы для получения соответствующих дат
...
def initialize_model(params=None):
    ...
    print(f"Инициализирован SGDClassifier с параметрами: {model.get_params()}")
    return model
...
def train_initial_model(model, X_train, y_train):
    ...
    global scaler # Используем глобальный scaler
    # Обучаем scaler на обучающей выборке и трансформируем её
    X_train_scaled = scaler.fit_transform(X_train)
    print("  Масштабирование завершено.")

    # --- Обучение модели ---
    print("  Обучение SGDClassifier...")
...
def evaluate_model(model, X_test, y_test, dataset_name="Тестовая выборка"):
    ...
    global scaler # Используем тот же scaler, что и для обучения
...
def perform_incremental_learning(model, X_test, y_test, test_dates):
    ...
    log_df.to_csv('incremental_learning_log_scaled.csv', index=False, encoding='utf-8-sig')
    print("Лог инкрементального обучения (масштабированный) сохранен в 'incremental_learning_log_scaled.csv'.")

def main():
    """Основная функция."""
    print("Начинаю обучение модели SGDClassifier с настройкой и инкрементальным обучением...")
    ...
    print("\nОбучение модели SGDClassifier с настройкой и инкрементальным обучением завершено.")
