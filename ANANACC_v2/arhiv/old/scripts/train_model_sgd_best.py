...
# --- Лучшие гиперпараметры SGDClassifier, найденные ранее ---
# Из результатов grid_search_sgd_results.csv
BEST_SGD_PARAMS = {
    'loss': 'hinge',         # SVM
    'alpha': 0.001,          # Сила регуляризации
    'learning_rate': 'constant', # Постоянная скорость обучения
    'eta0': 0.1,             # Начальная скорость обучения
...
def perform_incremental_learning_simulation(model, X_test, y_test, test_dates):
    ...
    log_df.to_csv('incremental_learning_log_sgd_best.csv', index=False, encoding='utf-8-sig')
    print("Лог инкрементального обучения (SGDClassifier лучший) сохранен в 'incremental_learning_log_sgd_best.csv'.")

def main():
    """Основная функция."""
    print("Начинаю обучение модели SGDClassifier с лучшими параметрами и инкрементальным обучением...")
    ...
    perform_incremental_learning_simulation(model, X_test, y_test, test_dates)
    ...
    print("\nОбучение модели SGDClassifier с лучшими параметрами и инкрементальным обучением завершено.")
