# combine_datasets_all_targets.py (исправленный)
import pandas as pd
import numpy as np
import os
...
def add_target_directions_for_all_tickers(df):
    ...
    # --- ИЗМЕНЕНИЕ 1: Создаем список для новых Series ---
    new_columns_series = []
    # -----------------------------------------------
    ...
    for ticker in tickers:
        ...
        if close_col in df.columns:
            ...
            # --- ИЗМЕНЕНИЕ 2: Вместо добавления столбца напрямую, сохраняем Series ---
            # df[target_col] = target_direction_series # <-- Старая строка
            new_columns_series.append(pd.Series(target_direction_series, name=target_col, index=df.index)) # <-- Новая строка
            # ------------------------------------------------------------
            added_targets += 1
        else:
    ...
    # --- ИЗМЕНЕНИЕ 3: Объединяем все новые Series за один раз ---
    if new_columns_series:
        print(f"  Объединяю {len(new_columns_series)} новых столбцов TARGET_DIRECTION...")
        # Используем pd.concat для объединения всех новых Series в один DataFrame
        new_columns_df = pd.concat(new_columns_series, axis=1)
        # Объединяем исходный df с новыми столбцами
        df = pd.concat([df, new_columns_df], axis=1)
        print("  Все новые столбцы TARGET_DIRECTION успешно добавлены.")
    else:
        print("  Не было создано ни одного нового столбца TARGET_DIRECTION.")
    # ------------------------------------------------------------
